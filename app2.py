...
uploaded_file = st.sidebar.file_uploader("Upload BMW Dataset (.csv)", type=["csv"])

if uploaded_file is not None:
    df_raw = load_and_clean_data(uploaded_file)
    st.success(" Dataset uploaded successfully.")
else:
    st.warning(" Please upload the BMW_sales_2010_2024.csv file to continue.")
    st.stop()
...
trans = col_trans.selectbox("Transmission", df_unencoded['Transmission'].unique())
st.sidebar.markdown("---")
...
if st.sidebar.button("Predict Current Price"):
    ...
    st.metric(label="Predicted Price Estimate", 
...
st.pyplot(fig6)
st.markdown("---")
st.info("Project completed successfully.")
